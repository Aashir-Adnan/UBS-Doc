...
- **`src/docs/sidebar.ts`** — `SIDEBAR: SidebarNode[]`, ported verbatim from the old `sidebars.js` `tutorialSidebar` array (this file is the **only** source of truth the app reads). The root `sidebars.js` still exists but is **dead** — nothing imports it, it is outside `tsconfig`'s `include`, and it never reaches `dist`. It is kept only to avoid a modify/delete merge conflict with `main` and is to be deleted once PR #12 merges; **do not edit it, and do not treat it as configuration**. `flattenSidebar()` (used by `routes.tsx` to resolve `/docs` → first doc) walks this tree.
