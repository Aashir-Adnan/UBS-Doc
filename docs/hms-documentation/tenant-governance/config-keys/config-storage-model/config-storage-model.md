...
Applied values on services/packages live in `hms_config` (`base_table='services'|'packages'`), written by the **CustomServices / CustomPackages** APIs (not the config-key admin API). The storage rule is **one entity per row, stored BARE** — for **both** `is_input` modes. A multi-value config (a selection of ids, or an entered array of ids/objects) becomes **several rows**, each holding a **bare** id or a **bare** JSON object — **never** an array-wrapped `[…]` cell. The reader re-aggregates the rows of a `(record_id, config_key_id)` back into the array the FE sent, so the round-trip is stable. The exact shape is decided by the value's `is_input` flag and `value_type`:

> **Tenant-scoped keys.** Four keys — `tax_profile`, `payment_timing`, `deposit_amount`, `pricing_rules` — were repointed to `target_table='tenants'` (`20260720_2`). Their applied value belongs to the **hotel/tenant** (`base_table='tenants'`, `record_id=<tenant_id>`) and is written via the [Tenant Configs](../../tenant-configs/tenant-configs.md) API, **not** CustomServices/CustomPackages. Since **2026-08-04** that API follows the **same** per-row model described below, in **both** `is_input` modes — the table and reader contract here apply to those keys too. (It previously kept an `is_input=1` value whole in a single row; see [tenant-configs.md](../../tenant-configs/tenant-configs.md) for the migration of legacy rows.)

| Kind | `is_input` | Stored shape |
|---|---|---|
| **Reference set** — a selection of option ids/objects (`is_input=0`: dropdowns, checkboxes, keyword chips, `deliver_unit`, `base_currency`, and object-valued references like `allowed_regions` / `blackout_dates`) | 0 | **exploded** — one **bare** row per element (an id as a bare scalar, a structured value as a bare JSON object). Each scalar row is JSON-valid (#210). **Empty selection → NO row is stored** (clear-retires): the write retires the record's existing active rows for that key and stores nothing — never NULL, never an `'[]'` sentinel. (Legacy `'[]'` rows are still tolerated by readers as an empty selection.) |
...
| **Multi-value entered** — an entered **array** of ids/objects: `attachment`/media, `*_api_form`, `keyword_chips`, … | 1 | **exploded** — one **bare** row per element (an id as a bare scalar, an object as a bare JSON object), each with its **own** `ar` mirror. e.g. media `[149,150]` → rows `"149"`,`"150"`; `tax_profile_api_form` `[190,191]` → rows `"190"`,`"191"`. |
| **Single object / form** — a single object value (`form`, e.g. `physical_dimension`) | 1 | the whole object in **one** bare row. |
| **`form_inputs`** (field-definition doc) | 1 | the **one** exception that is never exploded — the whole doc stays a single row. |

Readers re-aggregate the exploded rows per `(record_id, config_key_id)` back into a value array — **for both `is_input` modes**: `is_input=0` resolves each id to its option value via the reader contract in §2; `is_input=1` returns one `{en, ar}` per row (each row's bare value + its own `ar`). A single-valued key has one row, so its read is unchanged. This is the write-side counterpart to the option-storage model above — see the `normalizeConfigValue` (`splitConfigValueToRows` / `splitIsInput1Rows`) helpers in the CustomServices/CustomPackages writers.
...
This mirrors the [Tenant Configs](../../tenant-configs/tenant-configs.md) clear-retires contract; every stored row holds a bare value (no array wrapper) and empties are cleared the same way.
