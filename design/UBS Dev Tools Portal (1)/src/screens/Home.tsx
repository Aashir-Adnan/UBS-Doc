...
const DOCS = [
  { label: 'Getting Started', sub: 'Onboarding guide for new engineers', icon: '🚀' },
  { label: 'API Reference', sub: 'Full REST & GraphQL documentation', icon: '📡' },
  { label: 'Authentication', sub: 'OAuth2, tokens, and RBAC overview', icon: '🔐' },
  { label: 'Database Schema', sub: 'ERD, migrations, and seed data', icon: '🗄️' },
  { label: 'Deployment Guide', sub: 'CI/CD pipelines and environments', icon: '🚢' },
  { label: 'Architecture', sub: 'System design and service topology', icon: '🏗️' },
]
...
export default function Home({ navigate, theme }: Props) {
  const d = theme === 'dark'
        ...
        <div className="grid grid-cols-[1fr_340px] gap-8 items-start">
          ...
          <div className={c(card(theme), 'overflow-hidden')}>
            <div className={c('px-6 py-4 border-b flex items-center justify-between', divider(theme))}>
              ...
              <button className={c('text-xs font-semibold text-indigo-500 hover:text-indigo-600 tr flex items-center gap-1')}>
